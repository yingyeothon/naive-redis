...
fixture(`simple-push-pop`, async connection => {
  ...
  expect(await lindex(connection, testKey, 0)).toEqual(testValue(0));
  expect(await lindex(connection, testKey, 1)).toEqual(testValue(1));

  expect(await lpop(connection, testKey)).toEqual(testValue(0));
  expect(await llen(connection, testKey)).toBe(1);
  expect(await lindex(connection, testKey, 0)).toEqual(testValue(1));

  expect(await lpop(connection, testKey)).toEqual(testValue(1));
  expect(await llen(connection, testKey)).toBe(0);
  expect(await lindex(connection, testKey, 0)).toBeNull();
  expect(await lpop(connection, testKey)).toBeNull();
});
